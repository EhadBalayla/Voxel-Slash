...
uint32_t VKUtils::findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties) {
	...
	vkGetPhysicalDeviceMemoryProperties(GEditor->m_Renderer.GetPhysicalDevice(), &memProps);
...
void VKUtils::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory) {
	...
	if (vkCreateBuffer(GEditor->m_Renderer.GetDevice(), &bufferInfo, nullptr, &buffer) != VK_SUCCESS) {
		throw std::runtime_error("couldn't create buffer");
	}

	VkMemoryRequirements memoryRequirements{};
	vkGetBufferMemoryRequirements(GEditor->m_Renderer.GetDevice(), buffer, &memoryRequirements);
	...
	if (vkAllocateMemory(GEditor->m_Renderer.GetDevice(), &memoryAllocInfo, nullptr, &bufferMemory) != VK_SUCCESS) {
		throw std::runtime_error("failed to allocate memory");
	}

	vkBindBufferMemory(GEditor->m_Renderer.GetDevice(), buffer, bufferMemory, 0);
}
...
VkCommandBuffer VKUtils::BeginSingleUseCommandBuffer() {
	...
	allocInfo.commandPool = GEditor->m_Renderer.GetCommandPool();
	allocInfo.commandBufferCount = 1;

	VkCommandBuffer commandBuffer;
	vkAllocateCommandBuffers(GEditor->m_Renderer.GetDevice(), &allocInfo, &commandBuffer);
...
void VKUtils::EndSingleUseCommandBuffer(VkCommandBuffer commandBuffer) {
	...
	vkQueueSubmit(GEditor->m_Renderer.GetGraphicsQueue(), 1, &submitInfo, VK_NULL_HANDLE);
	vkQueueWaitIdle(GEditor->m_Renderer.GetGraphicsQueue());

	vkFreeCommandBuffers(GEditor->m_Renderer.GetDevice(), GEditor->m_Renderer.GetCommandPool(), 1, &commandBuffer);
}
...
void VKUtils::createImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory) {
	...
	if (vkCreateImage(GEditor->m_Renderer.GetDevice(), &imageInfo, nullptr, &image) != VK_SUCCESS) {
		throw std::runtime_error("couldn't create image");
	}

	VkMemoryRequirements memReqs;
	vkGetImageMemoryRequirements(GEditor->m_Renderer.GetDevice(), image, &memReqs);
	...
	if (vkAllocateMemory(GEditor->m_Renderer.GetDevice(), &imageMemInfo, nullptr, &imageMemory) != VK_SUCCESS) {
		throw std::runtime_error("couldn't allocate memory for the image");
	}

	vkBindImageMemory(GEditor->m_Renderer.GetDevice(), image, imageMemory, 0);
}
...
VkImageView VKUtils::createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags) {
	...
	if (vkCreateImageView(GEditor->m_Renderer.GetDevice(), &createInfo, nullptr, &imageView) != VK_SUCCESS) {
