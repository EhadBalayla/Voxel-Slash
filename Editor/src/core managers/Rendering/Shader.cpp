...
void Shader::LoadShader(const char* vertexPath, const char* fragmentPath, PipelineType type) {
	...
	if (type == PipelineType::D3) { //where we enable depth testing
	...
	if (vkCreateGraphicsPipelines(GEditor->m_Renderer.GetDevice(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &graphicsPipeline) != VK_SUCCESS) {
		throw std::runtime_error("failed to create graphics pipeline");
	}


	vkDestroyShaderModule(GEditor->m_Renderer.GetDevice(), fragmentModule, nullptr);
	vkDestroyShaderModule(GEditor->m_Renderer.GetDevice(), vertexModule, nullptr);
}
void Shader::UnloadShader() {
	vkDestroyPipeline(GEditor->m_Renderer.GetDevice(), graphicsPipeline, nullptr);
}


void Shader::Bind() {
	vkCmdBindPipeline(GEditor->m_Renderer.GetFrameCommandBuffer(), VK_PIPELINE_BIND_POINT_GRAPHICS, graphicsPipeline);
}
...
VkShaderModule createShaderModule(const std::vector<char>& code) {
	...
	if (vkCreateShaderModule(GEditor->m_Renderer.GetDevice(), &createInfo, nullptr, &module) != VK_SUCCESS) {
