...
void Window::StartFrame() {
    vkWaitForFences(m_Context.GetDevice(), 1, &m_Swapchain.inFlightFences[GEditor->m_Renderer.CurrentFrame], VK_TRUE, UINT64_MAX);
    vkResetFences(m_Context.GetDevice(), 1, &m_Swapchain.inFlightFences[GEditor->m_Renderer.CurrentFrame]);

    vkAcquireNextImageKHR(m_Context.GetDevice(), m_Swapchain.swapchain, UINT64_MAX, m_Swapchain.imageAvailableSemaphores[GEditor->m_Renderer.CurrentFrame], VK_NULL_HANDLE, &m_Swapchain.imageIndex);

    vkResetCommandBuffer(GEditor->m_Renderer.GetFrameCommandBuffer(), 0);

    VkCommandBufferBeginInfo beginInfo{};
    beginInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;

    if(vkBeginCommandBuffer(GEditor->m_Renderer.GetFrameCommandBuffer(), &beginInfo) != VK_SUCCESS) {
	...
	vkCmdSetViewport(GEditor->m_Renderer.GetFrameCommandBuffer(), 0, 1, &viewport);

	VkRect2D scissor{};
	scissor.offset = { 0, 0 };
	scissor.extent = m_Swapchain.swapchainImageExtent;
	vkCmdSetScissor(GEditor->m_Renderer.GetFrameCommandBuffer(), 0, 1, &scissor);

	vkCmdBeginRenderPass(GEditor->m_Renderer.GetFrameCommandBuffer(), &renderPassInfo, VK_SUBPASS_CONTENTS_INLINE);
}
void Window::NextFrame() {
	vkCmdEndRenderPass(GEditor->m_Renderer.GetFrameCommandBuffer());

    if (vkEndCommandBuffer(GEditor->m_Renderer.GetFrameCommandBuffer()) != VK_SUCCESS) {
	...
	submitInfo.pWaitSemaphores = &m_Swapchain.imageAvailableSemaphores[GEditor->m_Renderer.CurrentFrame];
	submitInfo.pWaitDstStageMask = waitStages;
	submitInfo.commandBufferCount = 1;
	submitInfo.pCommandBuffers = &GEditor->m_Renderer.GetFrameCommandBuffer();
	submitInfo.signalSemaphoreCount = 1;
	submitInfo.pSignalSemaphores = &m_Swapchain.renderingFinishedSemaphores[m_Swapchain.imageIndex];

	if (vkQueueSubmit(GEditor->m_Renderer.GetGraphicsQueue(), 1, &submitInfo, m_Swapchain.inFlightFences[GEditor->m_Renderer.CurrentFrame]) != VK_SUCCESS) {
	...
	vkQueuePresentKHR(GEditor->m_Renderer.GetPresentQueue(), &presentInfo);
	
	GEditor->m_Renderer.CurrentFrame = (GEditor->m_Renderer.CurrentFrame + 1) % m_Context.MAX_FRAMES_IN_FLIGHT;
}
